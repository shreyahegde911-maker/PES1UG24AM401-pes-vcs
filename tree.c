...
static int write_tree_level(Index *idx, const char *prefix, ObjectID *out_id) {
    Tree tree;
    tree.count = 0;
    ...
    for (int i = 0; i < idx->count; i++) {
        ...
        if (prefix) name = e->path + strlen(prefix);
        ...
        if (!slash) {
            ...
            ObjectID subtree_id;
            if (write_tree_level(idx, new_prefix, &subtree_id) != 0)
                return -1;
...
int tree_from_index(ObjectID *id_out) {
    ...
    if (write_tree_level(&idx, NULL, id_out) != 0)
        return -1;
